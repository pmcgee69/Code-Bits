...
struct lval_common_ref<A, B, X, Y, std::enable_if_t<
    std::is_reference<cond_res_t<copy_cv_t<X, Y>&, copy_cv_t<Y, X>&>>::value>>
{
...
struct common_ref<A&&, B&&, X, Y, std::enable_if_t<
    std::is_convertible<A&&, rref_cr_helper_t<X, Y>>::value &&
    std::is_convertible<B&&, rref_cr_helper_t<X, Y>>::value >>
{
